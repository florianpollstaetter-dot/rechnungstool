...
export default function BankPage() {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (rows.length === 0) { alert("Keine Transaktionen in der Datei gefunden."); return; }
        ...
        <table className="min-w-full divide-y divide-[var(--border)]">
          <thead className="bg-[var(--background)]">
            <tr>
              ...
              <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase">Empfaenger/Auftraggeber</th>
              ...
              <th className="px-3 py-3 text-center text-xs font-medium text-gray-500 uppercase">Match</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[var(--border)]">
            {filteredTxs.length === 0 && (
              <tr><td colSpan={6} className="px-3 py-8 text-center text-gray-500">Noch keine Transaktionen. Lade einen Kontoauszug als CSV hoch.</td></tr>
            )}
            {filteredTxs.map((tx) => {
              ...
              const matchLabel = tx.match_status === "confirmed" ? "Bestaetigt"
                : tx.match_status === "auto_matched" ? `Auto (${tx.match_confidence}%)`
                : tx.match_status === "rejected" ? "Abgelehnt"
                : "Offen";
                  ...
                  <td className="px-3 py-3 text-center">
                    <div className="flex items-center justify-center gap-1">
                      ...
                      {tx.match_status === "auto_matched" && (
                        <>
                          <button onClick={() => handleConfirmMatch(tx.id)} className="text-emerald-400 hover:text-emerald-300 text-xs" title="Bestaetigen">✓</button>
                          <button onClick={() => handleRejectMatch(tx.id)} className="text-rose-400 hover:text-rose-300 text-xs" title="Ablehnen">✗</button>
                        </>
                      )}
                    </div>
                  </td>
