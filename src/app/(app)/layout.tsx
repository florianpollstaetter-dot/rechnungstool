        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-14">
            <div className="flex items-center gap-3">
              {/* Orange Octo brand mark — always visible */}
              ...
              {/* Active company — dropdown when multiple, static when one */}
              {showCompanyDropdown ? (
                <div className="relative">
                  ...
                  >
                    ...
                    <span className="text-xs font-medium text-[var(--text-secondary)] hidden sm:block">{company.name.split(" ")[0]}</span>
                  ...
                  {showCompanySwitcher && (
                    <>
                      ...
                      <div className="absolute left-0 top-full mt-1 z-50 bg-[var(--surface)] border border-[var(--border)] rounded-lg shadow-xl py-1 min-w-[200px]">
                ...
                <div className="flex items-center gap-1.5 px-2">
                  <Image src={company.logo_path} alt={company.name} width={24} height={24} className="rounded" style={{ filter: "var(--logo-filter)" }} />
                  <span className="text-xs font-medium text-[var(--text-muted)] hidden sm:block">{company.name.split(" ")[0]}</span>
                </div>
              )}
              {userName && <span className="text-[10px] sm:text-xs text-[var(--text-muted)] italic hidden lg:inline">You rock my world, <strong className="text-[var(--text-primary)] not-italic">{userName}</strong>!</span>}
              {/* Desktop nav */}
