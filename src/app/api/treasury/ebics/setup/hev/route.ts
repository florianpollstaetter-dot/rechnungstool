// ORA-2308 — Step 4 / HEV-Probe + final activation. Requires version 3.0
// (H005 or "EBICS 3.x") — anything else fails the wizard.

import { NextResponse } from "next/server";
import { adminGate } from "../_lib/admin-gate";
import { EbicsSetupError, runHevProbe } from "@/lib/treasury/ebics";
...
export async function POST(request: Request) {
  ...
  try {
    const row = await runHevProbe(ctx.service, {
