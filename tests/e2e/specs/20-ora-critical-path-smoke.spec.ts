...
//   3. Storno money-path: click Stornieren on a finalized invoice, confirm in
//      the in-app modal (ORA-2466 replaced the native confirm() — which blocked
//      the CDP channel and looked like a "freeze" — with a testable modal),
//      then assert the original flips to `storniert` AND a `STORNO zu …`
...
test.describe("ORA-2465: Critical-path smoke (Korrekturphase)", () => {
  test("login + orange branding + Storno money-path persists, no critical console errors", async ({ page }) => {
    ...
    // ORA-2466 regression guard: the Stornieren flow must NOT raise a native
    // dialog (that was the freeze — native confirm() blocks the CDP channel).
    // The fix uses an in-app modal, so any native dialog here is a regression.
    ...
    const stornoBtn = page.locator('[data-testid="storno-button"]');
    await expect(stornoBtn).toHaveCount(1);
    await stornoBtn.first().click();

    // ORA-2466: the in-app confirmation modal opens (no native dialog / freeze).
    const modal = page.locator('[data-testid="storno-modal"]');
    await expect(modal).toBeVisible({ timeout: 15000 });
    await modal.locator('[data-testid="storno-confirm"]').click();
    ...
    expect(nativeDialogSeen, "ORA-2466 regression: native confirm() dialog fired").toBe(false);
